label day3s1:
    scene bedroom dark with dissolve

    "Maria! My sister, where is she?"

    play sound fabric
    pause 1.0
    play sound opendoor
    "I awake with a start, tripping over my bed sheets as I rush to her door. I shove the door open."

    scene hallway with dissolve
    pause 0.5
    play sound opendoor
    scene sisroom with dissolve
    show sis worry1 with dissolve


    voice "3-1-1.mp3" #amree
    sis "H-hey? Something up, did I do something wrong?"

    play music bgmsis fadeout 1.0 fadein 0.0

    "She recoils at my sudden interruption of her morning affairs, frozen with a sock half on her foot."

    voice "3-1-2.mp3" #potato
    pro "Yeah, yeah, everything's okay. I thought something had happened to you. Must've been a nightmare or something."

    "I reach over to her, pulling her into an embrace."

    show sis sadb

    voice "3-1-3.mp3" #amree
    sis "I, uhh, okay…"

    "She hugs back with a fervor that I wasn't quite expecting."

    "That's probably a result of a lack of attention; I think she really craves affection, something she doesn't really get from the rest of the family."

    "So… during the few times I humor her, she's on me tighter than a corset."

    "It's… confusing. But I don't hate it. It's fun for the both of us."

    "She pulls away, and looks at me inquisitively."

    show sis worry2

    voice "3-1-4.mp3" #amree
    sis "Are you okay? You've been acting kind of funny."

    voice "3-1-5.mp3" #potato
    pro "Who, me? No, no, nothing's wrong. Don't worry about me, I'm fine, I promise."

    show sis worry1

    voice "3-1-6.mp3" #amree
    sis "If you say so…"

    "She trails off, completely unconvinced. I'm having enough trouble convincing myself, I really doubt I could say anything more to help alleviate her anxieties. It's best to move on, avoid the situation altogether."

    voice "3-1-7.mp3" #potato
    pro "Almost ready for school?"

    show sis happy

    voice "3-1-8.mp3" #amree
    sis "Yep."

    voice "3-1-9.mp3" #potato
    pro "Okay, I'll be waiting by the door."

    show sis happy2

    voice "3-1-10.mp3" #amree
    sis "Mmkay!"

    "I back out of the room after patting her on the head reassuringly."

    hide sis
    with dissolve
    scene bedroom
    with dissolve

    play sound fabric
    "Now to return to my room to get dressed - something I forgot to do in my rush to find my sister."

    "Ugh, I hate this uniform. Skirts are just… I don't like them. Guess I'll force myself to wear it anyways..."

    scene hallway with dissolve
    play sound slamdoor
    pause 0.5
    scene livingroom with dissolve
    pause 0.5
    
    scene kitchen
    with dissolve

    "I walk to the kitchen, grabbing a decent sized mug to fit some coffee in."

    "As expected, my brother isn't ready for school just yet. I peeked in his room when I passed it, and he's still sitting in his bed, looking at his phone."

    "His backpack was still a mess, books strewn around his room without a single care. His textbooks were worn and faded, and his lined composite notebooks were all crushed and torn."

    "That boy, he needs to get his shit together. Thankfully, my mother isn't out here to notice."

    "I pop in some toast, buttering it for my sister to eat on her way to school."

    "My only request is for a slow, quiet, drama free day. I don't think I can take much more and remain sane."

    "If it weren't for the few bright lights in my life, God knows where I'd be."

    "At least I have a good amount of time with Lauren to look forward to. The research going into costume design is relatively boring, but with her? I'm sure it'll go over quite well."

    "It's a good thing, having someone by my side. It forces me to get real work done."

    "...It also makes it that much less lonely."

    "I eat my own serving of toast before wrapping the rest in a paper towel. I don't want any crumbs to fall onto the floor."

    "My mother cleans the kitchen meticulously, and she'd prefer it to stay that way. Can't say I blame her. I can only blame her reactions to when things don't quite go her way."

    "Looking at the time impatiently, I yell out to my sister."

    voice "3-1-11.mp3" #potato
    pro "Hey, sis, time to go!"

    voice "3-1-12.mp3" #amree
    sis "Sorry!"

    show sis worry1
    with dissolve

    play sound slamdoor
    "Her door closes, and she approaches me."

    voice "3-1-13.mp3" #potato
    pro "Girl, you take forever. You're not even old enough to put makeup on, what on Earth takes you so long?"

    show sis worry2
    voice "3-1-14.mp3" #amree
    sis "I'm just a little sleepy."

    voice "3-1-15.mp3" #potato
    pro "Alright, well, take some toast and let's hit the road. Mom'll kill me if you're late for class too."

    hide sis
    with dissolve

    scene siswalk
    with dissolve
    play ambience suburb fadein 2.0

    "We hit the open road."

    show sis happy
    with dissolve

    voice "3-1-16.mp3" #amree
    sis "Thanks for the food…"

    voice "3-1-17.mp3" #potato
    pro "Yeah, no prob. Toast's super easy to make anyways."

    show sis worry2

    voice "3-1-18.mp3" #amree
    sis "I guess. Why didn't Mom make food for us?"

    voice "3-1-19.mp3" #potato
    pro "I'm not sure. She might be busy getting groceries, or she's catching some sleep, or something like that. I wouldn't worry too much about it."

    show sis worry1

    voice "3-1-20.mp3" #amree
    sis "R-really? I worry about it."

    voice "3-1-21.mp3" #potato
    pro "Why? Why would you worry about her? She's an adult, she can handle herself."

    show sis sad
    voice "3-1-21_2.mp3" #amree
    sis "Mom is… she's really strict, but she still takes care of us. I… I don't wanna forget that."

    show sis sad2
    voice "3-1-21_3.mp3" #amree
    sis "She must be so stressed all of the time. It must be hard so… I wanna help Mom too."

    voice "3-1-21_4.mp3"
    sis "She loves us and… I love her too."

    voice "3-1-21_5.mp3"
    pro "Huh. She has a funny way of showing it…"

    show sis worry1
    voice "3-1-21_6.mp3"
    pro "...I guess you wear your heart on your sleeve. You always were the purest little cinnamon roll."

    show sis happy2
    voice "3-1-21_7.mp3"
    sis "Heheh! It's cuz I'm sweet, right??"

    "That got a laugh out of her."

    show sis worry2

    voice "3-1-21_8.mp3"
    sis "Still, it… it's hard, not to worry, I mean…"

    voice "3-1-22.mp3" #amree
    sis "You're an adult, but I still worry about you…"

    voice "3-1-23.mp3" #potato
    pro "Well, uhh, you don't really need to."

    show sis worry1
    voice "3-1-24.mp3" #amree
    sis "Are you sure?"

    "Why would she worry about me? I'm a bit high strung, but it's not my job to be something of concern to her. Hell, I'd rather it be the opposite."

    "While there's drama around the house, I need to stay out of it - and I can't contribute anymore than I already have."

    "I'm the rock others can cling to in their time of need. That's what Lauren would say, at least."

    "'If things are going right for you, then make sure they go right for everyone else, too!'"

    "Yeah, that sounds about right for her."

    voice "3-1-25.mp3" #potato
    pro "Yeah, o-of course."

    #we're editing a few lines in this to make Maria sound more like a kid and adding some lines

    show sis happy2

    #VOICE REDUB (honestly this existing line take is perfect if we go into audacity and cut "DEAL?" from the end)
    voice "3-1-26.mp3" #amree
    sis "I'll still worry about you… and Mom, and Dad, and everyone else. I swear it! So, don't do anything to make me worry more, please."

    show sis happy
    #VOICE NEW
    voice "3-1-26_2.mp3" #amree
    sis "Promise?"

    "Maria extends her pinky to me."

    #VOICE REDUB
    voice "3-1-27.mp3" #potato
    pro "Promise."

    "We make the pinky promise. The pact is sealed…"


    show sis happy2
    #VOICE NEW
    voice "3-1-28.mp3" #amree
    sis "Okay! Pinkie promises are super important, so now you gotta!"

    #VOICE NEW
    voice "3-1-29.mp3" #potato
    pro "Hehe, yeah. I know."

    "I'll try to hold myself to that, but I might not have the energy. I'll have to wait and see, I suppose."

    stop ambience fadeout 2.0
    stop music fadeout 5.0
    hide sis
    with dissolve
    scene black
    with dissolve

    "The rest of the journey, and the subsequent class both are relatively uneventful..."
    "Right now, I'm mostly worried about Lauren and her... special assignment."
    "...Joy."

    jump day3s2
